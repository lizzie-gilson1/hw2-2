...
void gather_for_save(particle_t* parts, int num_parts, double size, int rank, int num_procs) {
    // Allocate memory to receive particles from all processors
    particle_t* recv_parts = nullptr;
    if (rank == 0) {
        // change to just be num_parts
        recv_parts = new particle_t[num_parts];
    }

    // Calculate the size of the received array
    int recv_size = num_parts * num_procs;
    ...
    if (rank == 0) {
        std::sort(recv_parts, recv_parts + recv_size,
                  [](const particle_t& a, const particle_t& b) { return a.id < b.id; });

        // Initiate non-blocking sends to other processes
        std::vector<MPI_Request> requests(num_procs - 1);
        for (int i = 1; i < num_procs; ++i) {
            MPI_Isend(recv_parts + i * num_parts, num_parts, PARTICLE, i, 0, MPI_COMM_WORLD, &requests[i - 1]);
        }

        // Wait for all non-blocking sends to complete
        MPI_Waitall(num_procs - 1, requests.data(), MPI_STATUSES_IGNORE);

        // Copy master's own particles back to parts array
        std::copy(recv_parts, recv_parts + num_parts, parts);

        // Clean up memory on master rank
        delete[] recv_parts;
    }
// NOTES:
// change recv_parts size to just be num_parts
    // separate rank == 0 from other ranks --> use one if statement instead of 2
 // Sort received particles by particle id (only master processor)
    // use gdb to debug
// Can/should use Gatherv, but alternatively could use only send/recieve
  // current copying is naive way of doing it, use iSend (MPI instruction), could speed up

...
void assign_particles_to_bins(particle_t* parts, int num_parts, double size, int rank, int num_procs) {
    // Determine the range of particles assigned to this process
    int start = rank * num_parts / num_procs;
    int end = (rank + 1) * num_parts / num_procs;

    // Calculate the range of bins covered by this process
    int binStartX = rank * binCountX / num_procs;
    int binEndX = (rank + 1) * binCountX / num_procs;
    int binStartY = 0; // Assuming 1D decomposition
    int binEndY = binCountY;

    // Clear bins assigned to this process
    for (int i = binStartX; i < binEndX; ++i) {
        for (int j = binStartY; j < binEndY; ++j) {
            ListNode* node = bins[i][j];
            while (node != nullptr) {
    ...
    // Assign particles to bins within the assigned range
    for (int i = start; i < end; ++i) {
        int binX = std::floor(parts[i].x / binSize);
        int binY = std::floor(parts[i].y / binSize);
        binX = std::max(binStartX, std::min(binX, binEndX - 1));
        binY = std::max(binStartY, std::min(binY, binEndY - 1));

        ListNode* newNode = new ListNode(i);
        newNode->next = bins[binX][binY];
        bins[binX][binY] = newNode;
    }
...
void simulate_one_step(particle_t* parts, int num_parts, double size, int rank, int num_procs) {
    ...
    // Move particles
    start = rank * num_parts / num_procs;
    end = (rank + 1) * num_parts / num_procs;
    for (int i = start; i < end; ++i) {
        move(parts[i], size);
    }
